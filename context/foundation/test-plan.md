...
- Runtime/browser: Chrome browser automation (Claude in Chrome) — available for manual verification; not used for automated tests; checked: 2026-06-01
...
- **Profile management pages** — Breeze scaffolding; Laravel's own test suite covers these. Re-evaluate if custom profile logic is added. (Source: interview Q5.)
- **Welcome/landing page** — static HTML with auth links; no logic worth testing. Re-evaluate if dynamic content is added. (Source: interview Q5.)
- **Visual snapshot tests** — break on every CSS change, catch nothing meaningful for this project's scale. Use functional assertions (element presence, form submission) instead. Re-evaluate if a design system with strict visual contracts is adopted. (Source: interview Q5.)
